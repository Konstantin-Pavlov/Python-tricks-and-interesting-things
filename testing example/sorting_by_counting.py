...
# отсортировать список, состоящий только из чисел в пределах от -100 до 100 включительно, сортировкой подсчетом
n = int(input())
...
# здесь - range(-100, 101) - подходят к середине списка, как бы с разных сторон. 
# Положительный с начала, отрицательный с конца
for i in range(-100, 101):  # перебор в пределах от -100 до 100 включительно
    for _ in range(count[i]):  # если по индексу i в count есть ненулевое значение, оно будет выведено ... 
        print(i, end=' ')  # ... количество раз соответсвующее значению count[i]